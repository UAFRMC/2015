...
public:
  ...
  speed_controller_t(const float pgain,const float igain,const float dgain,const int encoder_pin,int encoder_threshold,BTS_motor_t& motor):
    encoder_t(encoder_pin,encoder_threshold),motor(motor),pid(0,0,0),average_without_div(0),times_ptr(0),target_time(1000)
  {
  ...
  int update(int target,const bool speed_control)
  {
   ...
   // target == 63 (stop) .. 0 (full power)
    ...
    target_time=target+10;

    const int motor_min=68; // don't give less power than this (keep spinning)
    const int motor_max=115; // don't give more than this
    ...
    else
    {
      ...
      int32_t err=(int32_t)real_average-(int32_t)target_time;
      motor_value=err/2+motor_idle;
